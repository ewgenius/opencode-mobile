...
// Import culori for OKLCH color calculations
...
// Types
/**
 * @typedef {Object} ThemeSeedColors
 ...
 * @property {string} diffDelete
 */

/**
 * @typedef {Object} ThemeVariant
 * @property {ThemeSeedColors} seeds
 * @property {Record<string, string>} [overrides]
 */

/**
 * @typedef {Object} DesktopTheme
...
/**
 * Generate color scale from seed color
 * @param {string} seedHex
 * @param {boolean} isDark
 * @returns {string[]}
 */
function generateScale(seedHex, isDark) {
...
/**
 * Generate neutral scale from seed color
 * @param {string} seedHex
 * @param {boolean} isDark
 * @returns {string[]}
 */
function generateNeutralScale(seedHex, isDark) {
  const seed = hexToOklch(seedHex);
  const scale = [];
...
/**
 * Generate alpha scale from neutral scale
 * @param {string[]} neutralScale
 * @param {boolean} isDark
 * @returns {string[]}
 */
...
function resolveThemeVariant(variant, isDark) {
  ...
  tokens['border-weaker-selected'] = withAlpha(interactive[3], isDark ? 0.3 : 0.4);
  ...
  // Resolve CSS var references in overrides
  for (const [key, value] of Object.entries(tokens)) {
    if (typeof value === 'string' && value.startsWith('var(--')) {
      tokens[key] = resolveVar(value, tokens);
    }
  }
...
/**
 * Map desktop tokens to mobile-friendly structure
 * @param {Record<string, string>} tokens
 * @returns {Record<string, string>}
 */
function mapToMobileTokens(tokens) {
  return {
    // Background
    ...
    surfaceActive: tokens['surface-base-active'],
    surfaceRaised: tokens['surface-raised-base'],
    surfaceRaisedHover: tokens['surface-raised-base-hover'],
    surfaceInset: tokens['surface-inset-base'],
    surfaceBrand: tokens['surface-brand-base'],
    surfaceBrandHover: tokens['surface-brand-hover'],
    surfaceInteractive: tokens['surface-interactive-base'],
    surfaceInteractiveHover: tokens['surface-interactive-hover'],
    surfaceSuccess: tokens['surface-success-base'],
    ...
    textTertiary: tokens['text-weaker'],
    textStrong: tokens['text-strong'],
    textInvert: tokens['text-invert-base'],
    textInteractive: tokens['text-interactive-base'],
    textOnBrand: tokens['text-on-brand-base'],
    textOnInteractive: tokens['text-on-interactive-base'],
    textOnSuccess: tokens['text-on-success-base'],
    ...
    borderActive: tokens['border-active'],
    borderSelected: tokens['border-selected'],
    borderInteractive: tokens['border-interactive-base'],
    borderSuccess: tokens['border-success-base'],
    ...
    iconActive: tokens['icon-active'],
    iconDisabled: tokens['icon-disabled'],
    iconWeak: tokens['icon-weak-base'],
    iconStrong: tokens['icon-strong-base'],
    iconInteractive: tokens['icon-interactive-base'],
    iconSuccess: tokens['icon-success-base'],
    ...
    syntaxPrimitive: tokens['syntax-primitive'],
    syntaxVariable: tokens['syntax-variable'],
    syntaxProperty: tokens['syntax-property'],
    syntaxType: tokens['syntax-type'],
    syntaxConstant: tokens['syntax-constant'],
    syntaxSuccess: tokens['syntax-success'],
    ...
    avatarBackgroundOrange: tokens['avatar-background-orange'],
    avatarBackgroundPurple: tokens['avatar-background-purple'],
    avatarBackgroundCyan: tokens['avatar-background-cyan'],
    avatarBackgroundLime: tokens['avatar-background-lime'],
    avatarTextPink: tokens['avatar-text-pink'],
    avatarTextMint: tokens['avatar-text-mint'],
    avatarTextOrange: tokens['avatar-text-orange'],
    avatarTextPurple: tokens['avatar-text-purple'],
    avatarTextCyan: tokens['avatar-text-cyan'],
    avatarTextLime: tokens['avatar-text-lime'],
  };
}

/**
 * Load a theme from the opencode repo
 * @param {string} themeId
 * @returns {DesktopTheme}
 */
...
/**
 * Generate theme files for all themes
 */
